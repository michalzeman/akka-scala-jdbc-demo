package com.mz.example.actors.services.common

import akka.actor.{Actor, ActorLogging, Props}
import akka.pattern._
import akka.util.Timeout
import com.mz.example.actors.common.messages.messages.UnsupportedOperation
import com.mz.example.actors.repositories.common.messages.{Insert, Inserted, SelectById}
 ...
 *
  ...
  protected def create(entity: E): Future[Created] = {
    ...
    (repository ? Insert(entity)).mapTo[Inserted] onComplete {
      case Success(s) => {
  ...
  protected def findById(id: Long): Future[Found[E]] = {
    ...
    (repository ? SelectById(id)).mapTo[Option[E]] onComplete {
      case Success(s) => {
  ...
  protected def delete(entity: E): Future[Deleted] = {
    import com.mz.example.actors.repositories.common.messages.Delete

    val p = Promise[Deleted]
    (repository ? Delete(entity.id)).mapTo[Boolean] onComplete {
  ...
  protected def update(entity: E): Future[UpdateResult[E]] = {
    import com.mz.example.actors.repositories.common.messages.Update

    val p = Promise[UpdateResult[E]]
    (repository ? Update(entity)).mapTo[Boolean] onComplete {
      case Success(true) => {
